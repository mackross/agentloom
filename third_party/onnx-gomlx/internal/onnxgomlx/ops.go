...
func onnxDequantizeLinear(x, scale, xZeroPoint *Node, targetAxis int, outputDType dtypes.DType) *Node {
	...
	if xZeroPoint != nil {
		if !xZeroPoint.IsScalar() {
			if xZeroPoint.Rank() != 1 {
				exceptions.Panicf("DequantizeLinear: x_zero_point must be a scalar or 1D, got %s instead", xZeroPoint.Shape())
			}
			newZeroPointShape := x.Shape().Clone()
			for axis := range newZeroPointShape.Dimensions {
				if axis != targetAxis {
					newZeroPointShape.Dimensions[axis] = 1
				} else if newZeroPointShape.Dimensions[axis] != xZeroPoint.Shape().Dimensions[0] {
					exceptions.Panicf("DequantizeLinear: x_zero_point must have same dimension as the input axis %d (input shape=%s), got %s instead", targetAxis, x.Shape(), xZeroPoint.Shape())
				}
			}
			xZeroPoint = Reshape(xZeroPoint, newZeroPointShape.Dimensions...)
		}
