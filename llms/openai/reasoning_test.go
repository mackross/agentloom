...
func TestResponsesStreamerPreservesReasoningItems(t *testing.T) {
	...
	if len(projected.Items) != 1 || projected.Items[0].(threads.ReasoningItem).Provider != "openai.responses" {
	...
	if !ok || reasoning.Provider != "openai.responses" || reasoning.ID != "rs_1" || reasoning.Visibility != threads.ReasoningVisibilitySummary || reasoning.Summary != "brief rationale" || !bytes.Equal(reasoning.Opaque, raw) {
	...
	input, err := conversationInput(threads.Req{Items: []threads.Item{reasoning}}, reasoningProvider)
	if err != nil {
...
func TestResponsesStreamerConfiguresReasoningProvider(t *testing.T) {
	streamer := NewResponsesStreamerWithClient(openaiapi.Client{}, "")
	streamer.ReasoningProvider = "deepseek.responses"
	if got := streamer.Capabilities().Reasoning; got != threads.ReasoningForProvider("deepseek.responses") {
		t.Fatalf("reasoning strategy = %#v", got)
	}
	events := reasoningEvents(t, `{"type":"response.output_item.done","item":{"id":"rs_1","type":"reasoning","summary":[]}}`)
	var got threads.Item
	if _, _, err := streamer.streamResponseItems(&reasoningResponseStream{events: events, at: -1}, func(item threads.Item) error { got = item; return nil }); err != nil {
		t.Fatal(err)
	}
	if reasoning, ok := got.(threads.ReasoningItem); !ok || reasoning.Provider != "deepseek.responses" {
		t.Fatalf("reasoning item = %#v", got)
	}
}
