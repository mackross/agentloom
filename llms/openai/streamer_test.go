...
func TestRequestInputItemsMapsToolCallsAndResults(t *testing.T) {
	items, err := conversationInput(threads.Req{Items: []threads.Item{
...
func TestResolveFunctionCallUsesOutputItemMetadata(t *testing.T) {
	...
	recordFunctionCall(functionCalls, responses.ResponseOutputItemUnion{
		Type:   "function_call",
		ID:     "fc_1",
		CallID: "call_1",
		Name:   "calculator",
	...
	callID, name := functionCallIdentity(functionCalls, "fc_1", "")
...
func TestResponsesStreamerContract(t *testing.T) {
	streamertest.Run(t, openAIContractHarness{})
}

func TestResponsesStreamerSSEContract(t *testing.T) {
	streamertest.Run(t, openAISSEContractHarness{})
}
