...
- durable restore should reconstruct the exact persisted thread items and control-block state
...
- rollback operations for retained item history/state
...
- `Thread`
  - owns the append-only thread item history
...
- `controlBlock`
  ...
  - tracks thread state:
    ...
    - `receiving_stream`
    - `stream_complete`
  - derives outstanding tool calls from the thread item history
- `ThreadExecutor`
...
- `ToolsSnapshot`
  - durable control item in the thread item history
...
- durable restore should preserve exact thread state and item history
...
- this note does not require exact wire-level provider stream resumption; it only
  defines how the item history is recovered and what the next request may look like
...
Whether that restored history can continue without rollback depends on whether
the new provider can accept assistant-prefixed request history. That is a
model/executor question, not a durability question.
...
- `SetExecutor(x)` when the thread is not `idle`
  ...
  - install `x`
  - ask `x` to choose a recovery policy for the current thread items and control-block state
...
1. `requested`
   - `ToolCall` exists in the thread item history
   - no `ToolCallStarted`
   - no result
...
Otherwise a crash could still leave the thread history in the ambiguous state:
...
1. rollback boundary
2. handling of outstanding tool calls in the retained item history
...
- `Exact`
  - keep the restored item history and current control-block state as-is
...
- the item history would first say "this call was canceled/unknown during recovery"
...
7. after rollback, re-read outstanding tool calls from the retained item history
...
- if the item history contains no outstanding tool calls, nothing happens
...
- if policy keeps exact state, the attached executor can send using the retained item history

### `receiving_stream`

- exact recovery is only possible if the streamer can support the retained request history
...
Thread is restored in `receiving_stream` with partial assistant text in its item history.
...
- `Thread.ApplyRecoveryPolicy(...)`
- rollback operations for retained item history/control-block state
